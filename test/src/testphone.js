...
$(document).on("click", ".setServers", function(){
    ...
    for(let i=0; i<10; i++) $('.ServerList').slick('slickRemove',true) //テーブルの初期化
    $(".ServerList").attr("id", boxName) //サーバリストID設定

    for(let i=0; i<10; i++){
        const CopyTemp1 = $($("#template1").html()).clone()
        CopyTemp1.find(".Server").text("サーバー" + (i + Number(num)))
        CopyTemp1.find(".Server").attr("ServerID", (i + Number(num)))
...
$(document).on("click", ".btn", function(){
    ...
    const Data = {
        Server: $(this).closest("thead").find(".Server").attr("ServerID"),
...
function save_Storage(){
    ...
    $(".Servers").each(function(){
        const Server = $(this).find(".Server").attr("ServerID")
        let Data = [] //配列初期化
...
function load_Storage(){
    //先にサーバ選択させる
    if($(".ServerList").find('.Server').length <= 1){
    ...
    if(Storage != null){
        ...
        */
        ...
        $(".Servers").each(function(){
            ...
            const Servers = $(this)
            const Server = Servers.find(".Server").attr("ServerID")
...
function Rollback(){
    if(TMP.length != 0){
        ...
        $(".Servers").each(function(){
            if(TMP[n].Server == $(this).find(".Server").attr("ServerID")){
...
function setTimer(objBox){
    const Server = objBox.closest("thead").find(".Server").attr("ServerID")
